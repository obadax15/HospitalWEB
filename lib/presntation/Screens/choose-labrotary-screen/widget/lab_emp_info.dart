...
class Lab_Emp_Info extends StatefulWidget {
    const Lab_Emp_Info({Key? key, required this.id}) : super(key: key);

    final int id ;

    @override
    State<Lab_Emp_Info> createState() => _Lab_Emp_InfoState();
  }

  class _Lab_Emp_InfoState extends State<Lab_Emp_Info> {
    var details ;
    ...
    Widget build(BuildContext context) {
      ...
      return Scaffold(
        ...
        body: BlocBuilder<AddLabEmpCubit, Add_Lab_Emp_State>(
          builder: (context, state) {
            if (state.add_lab_emp_status == Add_Lab_Emp_Status.loading || BlocProvider.of<AddLabEmpCubit>(context).details==null) {
              return const Center(child: CircularProgressIndicator(color: MyColor.mykhli,),) ;
            }
            details = BlocProvider.of<AddLabEmpCubit>(context).details['nonMedical'] ;
            empoinfo = [
              details['fatherName'],
              details['fullName'],
                ...
                return Directionality(
                  ...
                  child: AlertDialog(
                    ...
                    content: Column(
                      ...
                      children: [
                        BlocListener<AddLabEmpCubit, Add_Lab_Emp_State>(
                          listener: (context, state) {
                            if (state.add_lab_emp_status == Add_Lab_Emp_Status.editdone) {
                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('done successfully'))) ;
                            }
                          },
                          child: InkWell(
                            onTap: () async {
                              await BlocProvider.of<AddLabEmpCubit>(context).deleteLabEmp(widget.id);
                            },
                            child: const Text(
                              'حذف حساب',
                              style: TextStyle(
                        ...
                        InkWell(
                          onTap: () {
                            Navigator.of(context).push(MaterialPageRoute(builder: (_) => Add_Lab_Forms(isediting: true , details: details,id: widget.id,))) ;
                          },
                          child: const Text(
