...
class Reception_Info extends StatefulWidget {
  const Reception_Info({Key? key, required this.id}) : super(key: key);

  final int id ;

  @override
  State<Reception_Info> createState() => _Reception_InfoState();
}

class _Reception_InfoState extends State<Reception_Info> {
  var details ;
    ...
    return Scaffold(
      ...
      body: BlocBuilder<ReceptionCubit, ReceptionState>(
        builder: (context, state) {
          if (state.receptionStatus == ReceptionStatus.loading || BlocProvider.of<ReceptionCubit>(context).details==null) {
            return const Center(child: CircularProgressIndicator(color: MyColor.mykhli,),) ;
          }
          details = BlocProvider.of<ReceptionCubit>(context).details['nonMedical'] ;
          empoinfo = [
            details['fatherName'],
            details['fullName'],
              ...
              return Directionality(
                ...
                child: AlertDialog(
                  ...
                  content: Column(
                    ...
                    children: [
                      BlocListener<ReceptionCubit, ReceptionState>(
                        listener: (context, state) {
                          if (state.receptionStatus == ReceptionStatus.editdone) {
                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('done successfully'))) ;
                          }
                        },
                        child: InkWell(
                          onTap: () async {
                            await BlocProvider.of<ReceptionCubit>(context).deleteReception(widget.id);
                          },
                          child: const Text(
                            'حذف حساب',
                            style: TextStyle(
                      ...
                      InkWell(
                        onTap: () {
                          Navigator.of(context).push(MaterialPageRoute(builder: (_) => EditReception(isediting: true , details: details,id: widget.id,))) ;
                        },
                        child: const Text(
